...
1. 고객은 가입 시 부여된 마일리지, 등급을 조회할 수 있다
...
1. 장애격리
    ...
    1. 마일리지 부여 기능이 과중되면 마일리지 부여 기능을 잠시동안 하지 않고 잠시후에 처리 하도록 유도한다  Circuit breaker, fallback
1. 성능
...
분석단계에서의 조건 중 하나로 멤버 탈퇴(member)-> 마일리지 삭제(mileage) 간의 호출은 동기식 일관성을 유지하는 트랜잭션으로 처리하기로 하였다. 호출 프로토콜은 이미 앞서 Rest Repository 에 의해 노출되어있는 REST 서비스를 FeignClient 를 이용하여 호출하도록 한다. 
